(function($) {
    $.fn.jquizzy = function(settings) {
        var defaults = {
            questions: null,
            startImg: 'assert/images/start.gif',
            endText: '已结束!',
            shortURL: null,
            sendResultsURL: null,
            resultComments: {
        ...
        answers = [],
        introFob = '	<div class="intro-container slide-container"><a class="nav-start" href="#"><font size="6">江西农业大学-软件学院</font><br/><br/><span><img src="assert/images/jxau.jpg"></span><br/><br/><br/>请认真完成测试题。准备好了吗？<br/><br/><span><img src="'+config.startImg+'"></span></a></div>	',
        exitFob = '<div class="results-container slide-container"><div class="question-number">' + config.endText + '</div><div class="result-keeper"></div></div><div class="notice">请完成题目！</div><div class="progress-keeper" ><div class="progress"></div></div>',
        contentFob = '',
        ...
        for (questionsIteratorIndex = 0; questionsIteratorIndex < config.questions.length; questionsIteratorIndex++) {
            ...
            if(config.questions[questionsIteratorIndex].answers.length == 0) {
            	contentFob += '<textarea rows="10" cols="65" id="textarea"></textarea>';
            }else {
        ...
        function checkAnswers() {
            var resultArr = [],
            flag = false;
            for (var int = 0; int < countIndex; int++) {
            	userAnswers[indexContents[int]] = userContents[int];
        ...
        superContainer.find('.next').click(function() {
        	var context = $(this).parents('.slide-container').find('textarea').val();
        	if(context) {
        		if(context.trim().length == 0) {
        			notice.fadeIn(300);
        			return false;
        		}else {
        			userContents[countIndex] = context;
        			indexContents[countIndex] = count;
        			countIndex++;
        		}
        	}else if ($(this).parents('.slide-container').find('li.selected').length === 0) {
                notice.fadeIn(300);
                return false;
            }
            notice.hide();
        ...
        superContainer.find('.final').click(function() {
            ...
            for (var i = 0,toLoopTill = results.length; i < toLoopTill; i++) {
                ...
                resultSet += '<div class="resultsview-qhover">' + config.questions[i].question +"<div style='color:red;'>正确答案："+ config.questions[i].tureAnswer+"</div>";
                resultSet += "<ul>";
